 ...
 */
public final class ButterflyEntity extends PathAwareEntity implements Flutterer {
    ...
    protected void copyComponentsFrom(final ComponentsAccess from) {
        this.copyComponentFrom(from, BSTLDataComponentTypes.BUTTERFLY_VARIANT);
    ...
    public static boolean canSpawn(final EntityType<ButterflyEntity> type, final WorldAccess world, final SpawnReason spawnReason, final BlockPos pos, final Random random) {
        return world.getBlockState(pos.down()).isIn(BSTLTags.BlockTags.BUTTERFLIES_SPAWNABLE_ON) && world.getBaseLightLevel(pos, 0) > 8;
    }
